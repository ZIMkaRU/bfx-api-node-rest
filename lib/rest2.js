...
class RESTv2 {
  ...
  generateToken (opts, cb) {
    ...
    params = _pickBy(params, (v) => !_isNil(v))
   ...
   * @param {Array<string>} params.payCurrencies           - currencies in which invoice accepts the payments, supported
  ...
  _getMockPayInvoiceList (params) {
    ...
    const data = Array(limit).fill(null).map((item, i) => {
      return {
        ...mockData,
        id: uuidv4(),
        t: end - (i * step)
   ...
   */
  payInvoiceList (params, cb) {
    return this._getMockPayInvoiceList(params)
  }
}
